...
const addGuru = async (req,res) =>{
    try {
        ...
        const checkGuruExists = await guruModel.findOne({email : email});

        if(checkGuruExists) {
            return res.status(200).send({message : "Guru already exists"});
        }
...
const assignSeeker = async (req,res) => {
    try {
        let gurus = await guruModel.find({}).sort({ flowCount: -1 });
...
const toggleTopGuru = async (req, res) => {
    try {
        ...
        let guru = await guruModel.findById(guruId);
        guru.top = !guru.top;
